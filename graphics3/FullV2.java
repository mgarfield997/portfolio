...
public class FullV2 extends JFrame implements GLEventListener, KeyListener
{	private GLCanvas myCanvas;
	private Material thisMaterial;
	private String[] vBlinn1ShaderSource, vBlinn2ShaderSource, fBlinn2ShaderSource, vObjShaderSource, fObjShaderSource;
	private String[] skyboxVertShaderSource, skyboxFragShaderSource;
	...
	private Point3D cameraLoc = new Point3D(0.0, 0.2, 10.0);
	private Point3D lightLoc = new Point3D(-3.8f, 2.2f, 1.1f);
	private Point3D objLoc = new Point3D(0.5, 0.5, -2.0);
	private Point3D patrickLoc = new Point3D(3.0, 0, 2.0);
	...
	private int numCubeVertices;
	
	private ImportedModel myObj;
	private int numObjVertices;
	private ImportedModel patrick;
	private int numPatrickVertices;
	...
	Point3D eyePoint = new Point3D(-cameraLoc.getX(), -cameraLoc.getY(), -cameraLoc.getZ());
	Point3D lookingAtPoint = new Point3D(torusLoc.getX(), torusLoc.getY(), torusLoc.getZ());
	...
	{	GL4 gl = (GL4) GLContext.getCurrentGL();
		...
		//skybox
		gl.glUseProgram(rendering_programSkybox);
		...
		mv_location = gl.glGetUniformLocation(rendering_programSkybox, "mv_matrix");
		proj_location = gl.glGetUniformLocation(rendering_programSkybox, "proj_matrix");
...
//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	...
	{	GL4 gl = (GL4) GLContext.getCurrentGL();
		...
		//draw patrick
		m_matrix.setToIdentity();
		m_matrix.translate(patrickLoc.getX(), patrickLoc.getY(), patrickLoc.getZ());
		m_matrix.scale(2.5, 2.5, 2.5);
		m_matrix.rotate(0, -90, 0);
		...
		int numVertsPatrick = patrick.getNumVertices();
		...
		gl.glDrawArrays(GL_TRIANGLES, 0, numVertsPatrick);
	}
//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	public void passTwo()
	{	GL4 gl = (GL4) GLContext.getCurrentGL();
	...
	{	GL4 gl = (GL4) GLContext.getCurrentGL();
		...
		//float[] material = {1.0f, 1.0f, 1.0f};
		myObj = new ImportedModel("../spongebob.obj");
		patrick = new ImportedModel("../patrick.obj");
		...
		joglObjTexture = loadTexture("Objects/sponge.png");
...
// -----------------------------
	...
	numCubeVertices = cube_vertices.length;
	...
	float[] cube_texture_coord =
		...
		Vertex3D[] patrickVertices = patrick.getVertices();
		numPatrickVertices = patrick.getNumVertices();
		
		float[] pvaluesPatrick = new float[numPatrickVertices*3];
		float[] tvaluesPatrick = new float[numPatrickVertices*2];
		float[] nvaluesPatrick = new float[numPatrickVertices*3];
		
		for (int i=0; i<numObjVertices; i++)
		{	pvaluesPatrick[i*3]   = (float) (patrickVertices[i]).getX();
			pvaluesPatrick[i*3+1] = (float) (patrickVertices[i]).getY();
			pvaluesPatrick[i*3+2] = (float) (patrickVertices[i]).getZ();
			tvaluesPatrick[i*2]   = (float) (patrickVertices[i]).getS();
			tvaluesPatrick[i*2+1] = (float) (patrickVertices[i]).getT();
			nvaluesPatrick[i*3]   = (float) (patrickVertices[i]).getNormalX();
			nvaluesPatrick[i*3+1] = (float) (patrickVertices[i]).getNormalY();
			nvaluesPatrick[i*3+2] = (float) (patrickVertices[i]).getNormalZ();
		}
		...
		//load the cube vertices into the 8th buffer 
		gl.glBindBuffer(GL_ARRAY_BUFFER, vbo[7]);
		FloatBuffer vertBufCube = Buffers.newDirectFloatBuffer(cube_vertices);
		gl.glBufferData(GL_ARRAY_BUFFER, vertBufCube.limit()*4,vertBufCube, GL_STATIC_DRAW); 
		
		//load the cube texture coordinates into the 9th buffer
		gl.glBindBuffer(GL_ARRAY_BUFFER, vbo[8]);
		FloatBuffer texBufCube = Buffers.newDirectFloatBuffer(cube_texture_coord);
		gl.glBufferData(GL_ARRAY_BUFFER, texBufCube.limit()*4,texBufCube, GL_STATIC_DRAW); 
		
		//load the patrick vertex coordinates into the 10th buffer
		gl.glBindBuffer(GL_ARRAY_BUFFER, vbo[9]);
		FloatBuffer vertBufPatrick = Buffers.newDirectFloatBuffer(pvaluesPatrick);
		gl.glBufferData(GL_ARRAY_BUFFER, vertBufPatrick.limit()*4, vertBufPatrick, GL_STATIC_DRAW);

		//load the patrick texture coordinates into the 6th buffer
		gl.glBindBuffer(GL_ARRAY_BUFFER, vbo[10]);
		FloatBuffer texBufPatrick = Buffers.newDirectFloatBuffer(tvaluesPatrick);
		gl.glBufferData(GL_ARRAY_BUFFER, texBufPatrick.limit()*4, texBufPatrick, GL_STATIC_DRAW);

		//load the patrick normal coordinates into the 7th buffer
		gl.glBindBuffer(GL_ARRAY_BUFFER, vbo[11]);
		FloatBuffer norBufPatrick = Buffers.newDirectFloatBuffer(nvaluesPatrick);
		gl.glBufferData(GL_ARRAY_BUFFER, norBufPatrick.limit()*4,norBufPatrick, GL_STATIC_DRAW); 
	...
	public static void main(String[] args) { new FullV2(); }
...
//-----------------
	...
	{	GL4 gl = (GL4) GLContext.getCurrentGL();
		...
		vObjShaderSource = util.readShaderSource("Shaders/vertObj.shader");
		fObjShaderSource = util.readShaderSource("Shaders/fragObj.shader");
		skyboxVertShaderSource = util.readShaderSource("Shaders/vert2.shader");
		skyboxFragShaderSource = util.readShaderSource("Shaders/frag2.shader");
		...
		int vSkyboxShader = gl.glCreateShader(GL_VERTEX_SHADER);
		int fSkyboxShader = gl.glCreateShader(GL_FRAGMENT_SHADER);
		...
		gl.glShaderSource(vSkyboxShader, skyboxVertShaderSource.length, skyboxVertShaderSource, null, 0);
		gl.glShaderSource(fSkyboxShader, skyboxFragShaderSource.length, skyboxFragShaderSource, null, 0);
		...
		gl.glAttachShader(rendering_programSkybox, vSkyboxShader);
		gl.glAttachShader(rendering_programSkybox, fSkyboxShader);

		gl.glLinkProgram(rendering_program1);
		gl.glLinkProgram(rendering_program2);
		gl.glLinkProgram(rendering_programObj);
		gl.glLinkProgram(rendering_programSkybox);
	}
	...
	public void keyInput() {
		...
		if (camera) {
			if (forward) {
				...
				//cameraLoc = cameraLoc.add(offset);
